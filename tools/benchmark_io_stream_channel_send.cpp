...
static void closed_callback(atbus::channel::io_stream_channel *channel,       // 事件触发的channel
                            atbus::channel::io_stream_connection *connection, // 事件触发的连接
                            int /*status*/,                                   // libuv传入的转态码
                            void *,                                           // 额外参数(不同事件不同含义)
                            size_t                                            // 额外参数长度
...
int main(int argc, char *argv[]) {
    ...
    if (io_stream_connect(&channel, addr, connect_callback, NULL, 0) < 0) {
        std::cerr << "connect to " << argv[1] << " failed." << uv_err_name(channel.error_code) << ":" << uv_strerror(channel.error_code)
                  << std::endl;
    ...
    for (int try_times = 0; try_times < 2; ++try_times) {
        ...
        if (res) {
            if (EN_ATBUS_ERR_BUFF_LIMIT == res) {
                ...
                ++conf.sum_send_err;
                fprintf(stderr, "io_stream_send error, ret code: %d. %s: %s\n", res, uv_err_name(connection->channel->error_code),
                        uv_strerror(connection->channel->error_code));
            }
        } else {
            ++conf.pending_send;
            ++conf.sum_seq;
